...
function bytesToHex(bytes: Uint8Array): string {
  ...
  for (let i = 0; i < bytes.length; i++) {
    const item = bytes[i].toString(16);
    if (item.length < 2) {
      output += `0${item}`;
    } else {
      output += item;
...
function canonicalizeHeaders(headers: CanonicalHeader[]): CanonicalHeaderResult {
  ...
  const result: CanonicalHeaderResult = {
    canonical,
    signedHeaders,
  };
  return result;
...
export async function buildAuthorization(params: AuthorizationParams): Promise<string> {
